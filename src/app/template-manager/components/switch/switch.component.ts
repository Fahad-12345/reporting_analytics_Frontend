...
export class SwitchComponent implements OnInit {
	...
	ngOnInit() {
		this.subscription1 = this.subject.switchRefresh.subscribe((res) => {
			if (res.length != 0) {
		...
		this.subscription2 = this.subject.refreshDefaultValue.subscribe((res) => {
			if (res && res.hasOwnProperty('index')) {
				if (this.object.uicomponent_name == res.uicomponent_name) {
					...
					this.optionSelect(this.object.options, false, res.index);
				}
		...
		if (!this.object.defaultChecked) {
			for (let i = 0; i < this.object.options.length; i++) {
				...
				if (this.object.options[i].defaultValue) {
					this.optionSelect(this.object.options, false, i);
					this.object.defaultChecked = true;
				}
			}
		}
	...
	public optionSelect(options, event, index) {
		if (event) {
		...
		if (this.object.options[index].selected) {
			if (this.object.options[index].linkedStatementCheck) {
				...
				this.object.answers.push({ answer: this.object.options[index].label });
			}
		}
	...
	isFilled() {
		if ((!(!this.layoutService.editorView && !this.layoutService.isShowEditor) &&
			this.object.is_required && this.object.answers.length == 0	 )
			&& 
			!(
