...
export class CalendarWeekViewComponent implements OnChanges, OnInit, OnDestroy {
  ...
  @Input() CurDocName: any;
  @Input() CurrDocFirstName: any;
  @Input() checkDay: any;
  ...
  returnItem(day, events, specId, clinicId) {
    let newEvents = []
    for (let e of events) {
      if (
        (e.event.specId == specId &&
          e.event.clinicId == clinicId &&
          !this.swaps) ||
        (e.event.specId == clinicId &&
          e.event.clinicId == specId &&
          this.swaps)
      ) {
        let newDay = day.date.toString().substring(0, 3);
        if (newDay == "Thu" || newDay == "Fri" || newDay == "Sat") {
          e.event["placement"] = "left";
        }
        else {
          e.event["placement"] = "right";
        }
        newEvents.push(e.event);
      }
    }
    return newEvents;
  }
    ...
    for (let i = 0; i < this.speciality.length; i++) {
      if (this.speciality[i].id == id) {
        ...
        if (this.speciality.length == 0) {
          this.speciality.push({ name: '', id: 0 })
        }
        return
      }
    }
  ...
  EventClicked(e,viewDate:Date) {	
	  debugger; 
	  let currentEvent={...e};
	  let _datelist=currentEvent.dateList&&currentEvent.dateList.find(datelist=>new Date(datelist.start_date).toString().substring(0, 15)==viewDate.toString().substring(0, 15));
	  currentEvent.current_dateList_event=_datelist
    this.supervisorService.currentDeleteAppointment = {...currentEvent};
	// this.subject.docAssignId = e.id;
	this.subject.docAssignId = (currentEvent.current_dateList_event&&currentEvent.current_dateList_event.available_doctor_id)|| null
    this.subject.specialityForDeleteModal = this.CurDocName.userBasicInfo.middle_name?this.CurDocName.userBasicInfo.first_name + ' ' + this.CurDocName.userBasicInfo.middle_name + ' ' + this.CurDocName.userBasicInfo.last_name:this.CurDocName.userBasicInfo.first_name + ' '  + this.CurDocName.userBasicInfo.last_name;
    this.subject.clinicForDeleteModal = e.facilityLocations?e.facilityLocations.facility.name +'-'+e.facilityLocations.name:''
	this.subject.clinicQualifierForDeleteModal = e.facilityLocations?e.facilityLocations.facility.qualifier +'-'+e.facilityLocations.qualifier:''
    this.subject.clinicIdForDeleteModal = e.clinicId
    this.subject.specIdForDeleteModal = e.spec_id;
    this.subject.recIdForDeleteModal = e.rec_id;
