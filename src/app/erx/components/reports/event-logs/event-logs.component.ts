...
export class EventLogsComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('CustomizeColumnModal') CustomizeColumnModal :ModalDirective;
	@ViewChild('eventLogsList') eventLogsListTable: DatatableComponent;
  ...
  ngOnInit() {
    ...
    this.eventLogsListtingTable = this.localStorage.getObject('eventLogsTableList' + this.storageData.getUserId());
  }

  ngAfterViewInit() {
		if (this.eventLogsListTable?._internalColumns) {
			this.columns = makeDeepCopyArray([...this.eventLogsListTable._internalColumns]);
			this.columns.forEach(element => {
				if(this.eventLogsListtingTable.length) {
					let obj = this.eventLogsListtingTable.find(x => x?.header === element?.name);
			...
			if(this.eventLogsListtingTable.length) {
				...
				this.eventLogsListtingTable.forEach((item, index) => {
  ...
  getActivityLog(filterCheck, IsExported?) {
    ...
    this.loadSpin=true;
    this.requestService.sendRequest(erx_url.event_log, 'get', REQUEST_SERVERS.erx_api_url,body)
  ...
  exportData()
  {
    let body =
      ...
      let exportParams='?offset=' +
		...
		body.user_id;
    this.loadSpin=true;
     this._service.EventlogExport('',exportParams).subscribe(result=>{
  ...
  openCustomoizeColumn(CustomizeColumnModal) {
	...
	onConfirm(click) {
		...
		if(click) {
			...
			this.localStorage.setObject('eventLogsTableList' + this.storageData.getUserId(), data);
		}
		...
		this.eventLogsListTable._internalColumns = columnsBody.filter(c => {
			return c.checked == true;
		});
		let groupByHeader = getIdsFromArray(this.modalCols, 'header'); // pick header property from alphabetical columns
		this.eventLogsListTable._internalColumns.sort(function (a, b) {
