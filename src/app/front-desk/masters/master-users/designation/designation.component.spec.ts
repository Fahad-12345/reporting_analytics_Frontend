...
describe('DesignationComponent', () => {
	...
	it('Should ngOnInit Test', () => {
		comp.designationForm = comp['fb'].group({
		...
		expect(comp.designationForm.value).toEqual({
			name: '',
			description: '',
		});
	...
	it('Should onSubmit When Subscribe successfull If designationForm valid', fakeAsync(() => {
		...
		comp.designationForm.controls.name.setValue('Mock Name');
		comp.designationForm.controls.description.setValue('Mock Description');
		let Given_Responce = {
			status: true,
			message: 'Users List',
			result: {
	...
	it('Should onSubmit When Subscribe UnSuccessfull If designationForm valid', fakeAsync(() => {
		debugger;
		comp.designationForm = comp['fb'].group({
			name: ['', [Validators.required, whitespaceFormValidation()]],
			description: [''],
	...
	it('Should onSubmit If designationForm invalid ', () => {
		comp.designationForm = comp['fb'].group({
			name: ['', [Validators.required, whitespaceFormValidation()]],
			description: [''],
		});
	...
	it('Should Test edit', () => {
		...
		expect(comp.headerText).toBe('Edit Designation');
	...
	it('Should Test save', () => {
		...
		expect(comp.headerText).toBe('Add Designation');
		expect(comp.submitText).toBe('Save & Continue');
		expect(comp.resetForm).toHaveBeenCalled();
		expect(comp.openModal).toHaveBeenCalled();
	});
	it('Should deleteOne When Subscribe successfull If designation have values', fakeAsync(() => {
		...
		comp.designations = [{id:1,name:'mock name',description:'mock description'}];
		...
		tick(15000);
		discardPeriodicTasks();
		expect(comp.designations.length).toBe(0);
	}));
	...
	it('Should closeModal function test when subscribe return empty responce	', fakeAsync(() => {
		...
		comp.designationForm.markAsTouched();
		comp.designationForm.markAsDirty();
	...
	it('Should closeModal function test when subscribe return not empty responce	', fakeAsync(() => {
		...
		comp.designationForm.markAsTouched();
		comp.designationForm.markAsDirty();
		spyOn(comp.designationForm,'reset');
        spyOn(canActivate_MockService, 'canDeactivate').and.returnValue(of([]));
		comp.closeModal();
		comp.modalRef.close();
        expect(comp.designationForm.reset).toHaveBeenCalled();
	}));
	it('Should closeModal function test when form invalid', () => {
		...
		comp.modalRef = comp['modalService'].open('paymentType', ngbModalOptions);
		comp.designationForm.markAsUntouched();
		// comp.designationForm.di();
		spyOn(comp.designationForm,'reset');
        spyOn(canActivate_MockService, 'canDeactivate').and.returnValue(of([]));
		comp.closeModal();
		comp.modalRef.close();
	...
	it('Should isDisableSaveContinue Test If form Invalid',()=>{
		comp.designationForm = comp['fb'].group({
			name: ['', [Validators.required, whitespaceFormValidation()]],
			description: [''],
		});
		comp.designationForm.controls.name.setValue('mock name');
		comp.designationForm.controls.description.setValue('mock description');
		let Result = comp.isDisableSaveContinue();
		debugger;
		expect(Result).toBeFalsy();
	});
	it('Should isDisableSaveContinue Test If valid',()=>{
		comp.designationForm = comp['fb'].group({
			name: ['', [Validators.required, whitespaceFormValidation()]],
			description: [''],
		});
