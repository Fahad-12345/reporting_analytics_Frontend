...
import { DepartmentMockService } from '@appDir/shared/mock-services/departmentMock.service';
import { ToastertMockService } from '@appDir/shared/mock-services/toasterMock.service';
...
describe('DepartmentComponent', () => {
	...
	let mock_departmentService = new DepartmentMockService();
	let mock_toasterService = new ToastertMockService();
	beforeEach(async(() => {
		TestBed.configureTestingModule({
			...
			providers: [
				...
				{ provide: DepartmentService, useValue: mock_departmentService },
				{ provide: ToastrService, useValue: mock_toasterService },
			],
		}).compileComponents();
	}));
	...
	it('Should ngOnInit Test', () => {
		comp.departmentForm = comp['fb'].group({
		...
		expect(comp.departmentForm.value).toEqual({
			name: '',
			description: '',
		});
	...
	it('Should getDeparments When Subscribe successfull', fakeAsync(() => {
		let Given_Responce = {
			status: true,
			message: 'Users List',
			result: {
	...
	it('Should onSubmit When Subscribe successfull If designationForm valid', fakeAsync(() => {
		comp.departmentForm = comp['fb'].group({
			name: ['', [Validators.required, whitespaceFormValidation()]],
			description: [''],
		});
		comp.departmentForm.controls.name.setValue('Mock Name');
		comp.departmentForm.controls.description.setValue('Mock Description');
		let Given_Responce = {
			status: true,
	...
	it('Should onSubmit When Subscribe UnSuccessfull If departmentForm valid', fakeAsync(() => {
		comp.departmentForm = comp['fb'].group({
			name: ['', [Validators.required, whitespaceFormValidation()]],
			description: [''],
		});
		...
		comp.departments = [];
		spyOn(request_MockService, 'sendRequest').and.throwError("My Error");
		comp.onSubmit();
		expect(request_MockService.sendRequest).toHaveBeenCalled();
		tick(15000);
		discardPeriodicTasks();
	...
	it('Should onSubmit If departmentForm invalid ', () => {
		comp.departmentForm = comp['fb'].group({
			name: ['', [Validators.required, whitespaceFormValidation()]],
			description: [''],
		});
		spyOn(comp['service'], 'touchAllFields');
		comp.onSubmit();
		expect(comp['service'].touchAllFields).toHaveBeenCalled();
	});
	...
	it('Should resetForm Test', () => {
		...
		spyOn(comp.departmentForm, 'reset');
		comp.resetForm();
		expect(comp.departmentForm.reset).toHaveBeenCalled();
	});
	...
	it('Should Test edit', () => {
		...
		expect(comp.headerText).toBe('Edit Department');
	...
	it('Should Test save', () => {
		...
		expect(comp.headerText).toBe('Add Department');
	...
	it('Should closeDepartment function test when subscribe return empty responce	', fakeAsync(() => {
		const formValue: any = {
			comments: 'comment',
			description: 'desc',
			name: 'name',
		...
		comp.departmentForm.markAsTouched();
		comp.departmentForm.markAsDirty();
        spyOn(canActivate_MockService, 'canDeactivate').and.returnValue(of(null));
		let ReturnValue = comp.closeDepartment();
        expect(ReturnValue).toBeUndefined();
	}));
	it('Should closeDepartment function test when subscribe return not empty responce	', fakeAsync(() => {
		const formValue: any = {
			comments: 'comment',
			description: 'desc',
			name: 'name',
		...
		comp.closeDepartment();
		comp.modalRef.close();
        expect(comp.departmentForm.reset).toHaveBeenCalled();
	}));
	it('Should closeDepartment function test when form invalid', () => {
		comp.departmentForm = comp['fb'].group({
			name: ['', [Validators.required, whitespaceFormValidation()]],
			description: [''],
		});
		...
		comp.modalRef = comp['modalService'].open('paymentType', ngbModalOptions);
		comp.departmentForm.markAsUntouched();
		spyOn(comp.departmentForm,'reset');
        spyOn(canActivate_MockService, 'canDeactivate').and.returnValue(of([]));
		comp.closeDepartment();
		comp.modalRef.close();
	...
	it('Should deleteOne When Subscribe successfull', fakeAsync(() => {
		...
		comp.departments = [{name:'mock name',description:'mock description'}];
		spyOn(comp.selection,'clear');
	...
	it('Should deleteOne When Subscribe successfull When _department not exists', fakeAsync(() => {
		let Confirmation_Responce = {
			resolved: true,
		};
		let Given_Row = {
		...
		spyOn(confirm_MockService, 'create').and.returnValue(of(Confirmation_Responce).pipe(delay(1)));
		spyOn(request_MockService, 'sendRequest').and.returnValue(of(Given_Row).pipe(delay(1)));
		// spyOn(mock_departmentService,'deleteOneDepartment').and.returnValue(false);
	...
	it('Should bulkDelete When Subscribe successfull When deleteMultipleDepartments False', fakeAsync(() => {
		let Confirmation_Responce = {
			resolved: true,
		};
		let Given_Row = {
		...
		spyOn(confirm_MockService, 'create').and.returnValue(of(Confirmation_Responce).pipe(delay(1)));
		spyOn(request_MockService, 'sendRequest').and.returnValue(of(Given_Row).pipe(delay(1)));
		// spyOn(mock_departmentService,'deleteMultipleDepartments').and.returnValue(false);
		spyOn(comp['toastrService'],'error');
		comp.bulkDelete();
		expect(confirm_MockService.create).toHaveBeenCalled();
		tick(15000);
	...
	it('Should deleteOne When Subscribe successfull & test catch block', fakeAsync(() => {
		let Confirmation_Responce = {
			resolved: true,
		};
		spyOn(comp['toastrService'],'error');
	...
	it('Should bulkDelete When Subscribe successfull & test catch block', fakeAsync(() => {
		let Confirmation_Responce = {
			resolved: true,
		};
		spyOn(comp['toastrService'],'error');
