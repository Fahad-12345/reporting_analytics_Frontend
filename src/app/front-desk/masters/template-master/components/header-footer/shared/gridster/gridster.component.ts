...
export class GridsterComponent implements OnInit, OnChanges, OnDestroy, GridsterComponentInterface {
	...
	constructor(
		...
		this.emptyCell = new GridsterEmptyCell(this, toastrService, subjectService, sanitizer);
	...
	ngOnInit(): void {
		this.subscription1 = this.subjectService.castResize.subscribe((res) => {
			if (res.length !== 0) {
				setTimeout(() => {
					this.onResize();
				}, 10);
				this.subjectService.resizeRefreshItem('');
			}
		});
	...
	setHorizontalTheme() {
		...
		if (
			...
			this.layoutService.template.sections[this.layoutService.lastI].dashboard.length
		) {
