...
import { PaymentActionTypeComponent } from './payment-action.component';
import { HttpClientTestingModule } from '@angular/common/http/testing';

describe('PaymentActionTypeComponent', () => {
	let comp: PaymentActionTypeComponent;
	let fixture: ComponentFixture<PaymentActionTypeComponent>;
	...
	beforeEach(async(() => {
		TestBed.configureTestingModule({
			declarations: [PaymentActionTypeComponent],
	...
	beforeEach(() => {
		fixture = TestBed.createComponent(PaymentActionTypeComponent);
	...
	it('Should createPaymentStatusSubmit', fakeAsync(() => {
		...
		comp.paymentActionTypeform = paymentFormSearch();
		comp.paymentActionTypeform.controls['name'].setValue(formValue.name);
		comp.paymentActionTypeform.controls['comments'].setValue(formValue.comments);
		comp.paymentActionTypeform.controls['description'].setValue(formValue.description);
		tick();
		expect(comp.loadSpin).toBeFalsy();
		comp.createPaymentStatusSubmit(comp.paymentActionTypeform.value);
		expect(comp.loadSpin).toBeFalsy();
	...
	it('Should createPaymentStatusSubmit function test when createPaymentStatusSubmit complete', () => {
		...
		comp.paymentActionTypeform = paymentFormSearch();
		comp.paymentActionTypeform.controls['name'].setValue(formValue.comments);
		comp.paymentActionTypeform.controls['comments'].setValue(formValue.description);
		comp.paymentActionTypeform.controls['description'].setValue(formValue.name);
	...
	it('Should createPaymentStatusSubmit function when Subscribe throw error ', fakeAsync(() => {
		...
		comp.modalRef = comp['modalService'].open('denialModel', ngbModalOptions);
		comp.paymentActionTypeform = paymentFormSearch();
		comp.paymentActionTypeform.controls['name'].setValue(formValue.comments);
		comp.paymentActionTypeform.controls['comments'].setValue(formValue.description);
		comp.paymentActionTypeform.controls['description'].setValue(formValue.name);
	...
	it('Should paymentActionTypeSetPage function test', () => {
		spyOn(comp, 'addUrlQueryParams');
		spyOn(comp, 'displayDenialUpdated');
		spyOn(comp['paymentStatusSelection'], 'clear');
		comp.paymentActionTypeform = comp.paymentFormSearch();
		const pageInfo = {
			offset: 0,
		};
		comp['PaymentActionTypePage'].pageNumber = pageInfo.offset;
		const pageNumber = comp['PaymentActionTypePage'].pageNumber + 1;
		comp.paymentActionTypeform.controls.name.setValue('Your name');
		comp.paymentActionTypeform.controls.comments.setValue('Your comments');
		comp.paymentActionTypeform.controls.description.setValue('Your description');
		const filters = checkReactiveFormIsEmpty(comp.paymentActionTypeform);
		comp['queryParams'] = {
			filter: !isObjectEmpty(filters),
			order: OrderEnum.ASC,
			per_page: comp['PaymentActionTypePage'].size || 10,
			page: pageNumber,
			pagination: true,
			order_by: 'name',
		};
		let queryparam = { per_page: comp['PaymentActionTypePage'].size, page: pageNumber };
		expect(comp['PaymentActionTypePage'].pageNumber).toBe(pageInfo.offset);
		...
		comp.paymentActionTypeform.controls['comments'].setValue(formValue.comments);
		comp.paymentActionTypeform.controls['description'].setValue(formValue.description);
		comp.paymentActionTypeSetPage(pageInfo);
	...
	it('Should displayDenialUpdated function test when subscribe throw error', fakeAsync(() => {
		...
		comp.paymentActionTypeSerarchForm = paymentFormSearch();
		comp.paymentActionTypeSerarchForm.controls['name'].setValue(formValue.comments);
		comp.paymentActionTypeSerarchForm.controls['comments'].setValue(formValue.description);
		comp.paymentActionTypeSerarchForm.controls['description'].setValue(formValue.name);
        spyOn(request_MockService, 'sendRequest').and.returnValue(throwError('error') );
	...
	it('Should closeModel function test when subscribe return empty responce	', fakeAsync(() => {
		...
		comp.paymentActionTypeform = comp['fb'].group({
			id: [''],
			name: ['', Validators.required],
			comments: [''],
			description: [''],
		});
		comp.paymentActionTypeform.markAsTouched();
		comp.paymentActionTypeform.markAsDirty();
	...
	it('Should closeModel function test when subscribe return not empty responce	', fakeAsync(() => {
		...
		comp.paymentActionTypeform.markAsTouched();
		comp.paymentActionTypeform.markAsDirty();
		spyOn(comp.paymentActionTypeform,'reset');
        spyOn(canDeactive_MockService, 'canDeactivate').and.returnValue(of([]));
		comp.closeModel();
		comp.modalRef.close();
        expect(comp.paymentActionTypeform.reset).toHaveBeenCalled();
	}));
...
// 
	...
	it('Should Test page limit function', () => {
		spyOn(comp, 'paymentActionTypeSetPage');
		comp.paymentStatusPageLimit('3');
		expect(comp.paymentActionTypeSetPage).toHaveBeenCalled();
		expect(comp['PaymentActionTypePage'].size).toBe(3);
	});
	...
	it('Should check Reset Button when paymentActionTypeSerarchForm From Empty function Test', () => {
		comp.paymentActionTypeSerarchForm = comp.paymentFormSearch();
		const retrunValue = isEmptyObject(comp.paymentActionTypeSerarchForm.value);
		comp.checkInputs();
		expect(retrunValue).toBe(true);
	});
	it('Should check Reset Button disabled when paymentActionTypeSerarchForm have values function Test', () => {
		comp.paymentActionTypeSerarchForm = comp.paymentFormSearch();
		comp.paymentActionTypeSerarchForm.controls.name.setValue('mock name');
		comp.paymentActionTypeSerarchForm.controls.comments.setValue('mock comments');
		comp.paymentActionTypeSerarchForm.controls.description.setValue('mock description');
		const retrunValue = isEmptyObject(comp.paymentActionTypeSerarchForm.value);
		comp.checkInputs();
	...
	it('Should Test closeModel when paymentActionTypeform dirty and touched false', fakeAsync(() => {
		const ngbModalOptions: NgbModalOptions = {
			backdrop: 'static',
			keyboard: false,
			windowClass: 'modal_extraDOc unique',
		...
		spyOn(comp.paymentActionTypeform,'reset');
		comp.paymentActionTypeform.reset();
		comp.closeModel();
		comp.modalRef.close();
		expect(comp.paymentActionTypeform.reset).toHaveBeenCalled();
	}));
	it('Should Test closeModel when paymentActionTypeform dirty and touched true', fakeAsync(() => {
	...
	it('Should Test closeModel when paymentActionTypeform dirty and touched true and subcribe throw error', fakeAsync(() => {
		// const spyOnMethod = spyOn(comp['CanDeactivateService'], 'canDeactivate').and.returnValue(of([]));
	...
	it('Should displayDenialUpdated with paymentStatusComingData empty', fakeAsync(() => {
		...
		expect(comp['PaymentActionTypePage'].totalElements).toBe(0);
    }));
	it('Should Test resetPaymentActionType function',()=>{
		spyOn(comp,'paymentActionTypeSetPage');
		spyOn(comp['fb'],'group');
		comp.paymentActionTypeSerarchForm = comp.paymentFormSearch();
		comp.resetPaymentActionType();
		expect(comp.paymentActionTypeSetPage).toHaveBeenCalled();
		expect(comp['fb'].group).toHaveBeenCalled();
	});
	it('Should Test updateDeniatSubmit function when subsribe run successfull',fakeAsync(()=>{
		...
		comp.modalRef = comp['modalService'].open('denialModel', ngbModalOptions);
		comp.paymentActionTypeform = comp.paymentFormSearch();
		comp.paymentActionTypeform.controls.name.setValue('mock name');
		comp.paymentActionTypeform.controls.comments.setValue('mock comments');
		comp.paymentActionTypeform.controls.description.setValue('mock description');
		spyOn(request_MockService, 'sendRequest').and.returnValue(of(formValue).pipe(delay(1)));
		comp.updateDeniatSubmit(comp.paymentActionTypeform);
        fixture.detectChanges();
		expect(comp.loadSpin).toBeTruthy();
        expect(request_MockService.sendRequest).toHaveBeenCalled();
        tick(15000);
	...
	it('Should Test onSavePaymentStatusSubmit function',()=>{
		...
		comp.paymentActionTypeSerarchForm.controls.comments.setValue('mock comments');
		comp.paymentActionTypeSerarchForm.controls.description.setValue('mock description');
		comp.onSavePaymentStatusSubmit(comp.paymentActionTypeSerarchForm.value);
		comp.modelTitle = 'Add';
	...
	it('Should Test openPaymentActionTypeModal function',()=>{
		spyOn(comp,'patchAddValues');
		comp.paymentActionTypeSerarchForm = comp.paymentFormSearch();
		comp.paymentActionTypeSerarchForm.controls.name.setValue('mock name');
		comp.paymentActionTypeSerarchForm.controls.comments.setValue('mock comments');
		comp.paymentActionTypeSerarchForm.controls.description.setValue('mock description');
		comp.openPaymentActionTypeModal(undefined,undefined,undefined);
		expect(comp.patchAddValues).toHaveBeenCalled();
	});
	it('Should Test openPaymentActionTypeModal function with row not undefined',()=>{
		spyOn(comp,'patchEditValues');
		comp.paymentActionTypeSerarchForm = comp.paymentFormSearch();
		comp.paymentActionTypeSerarchForm.controls.name.setValue('mock name');
		comp.paymentActionTypeSerarchForm.controls.comments.setValue('mock comments');
		comp.paymentActionTypeSerarchForm.controls.description.setValue('mock description');
		comp.openPaymentActionTypeModal(undefined,comp.paymentActionTypeSerarchForm.value,3);
		expect(comp.patchEditValues).toHaveBeenCalled();
	});
	it('Should Test openPaymentActionTypeModal function with row null',()=>{
		spyOn(comp,'patchAddValues');
		comp.paymentActionTypeSerarchForm = comp.paymentFormSearch();
		comp.paymentActionTypeSerarchForm.controls.name.setValue('mock name');
		comp.paymentActionTypeSerarchForm.controls.comments.setValue('mock comments');
		comp.paymentActionTypeSerarchForm.controls.description.setValue('mock description');
		comp.openPaymentActionTypeModal(undefined,null,3);
		expect(comp.patchAddValues).toHaveBeenCalled();
	});
