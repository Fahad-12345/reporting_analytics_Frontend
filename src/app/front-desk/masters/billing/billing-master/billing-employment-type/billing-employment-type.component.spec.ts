...
import { BillingEmploymentTypeComponent } from './billing-employment-type.component';
import { HttpClientTestingModule } from '@angular/common/http/testing';
describe('BillingEmploymentTypeComponent', () => {
	let comp: BillingEmploymentTypeComponent;
	let fixture: ComponentFixture<BillingEmploymentTypeComponent>;
	...
	beforeEach(async(() => {
		TestBed.configureTestingModule({
			declarations: [BillingEmploymentTypeComponent],
			...
			providers: [
				...
				// { provide: useValue: confirm_MockService },
			],
		}).compileComponents();
	}));

	beforeEach(() => {
		fixture = TestBed.createComponent(BillingEmploymentTypeComponent);
	...
	it('Should createFormSubmit', fakeAsync(() => {
		...
		comp.employmentTypeForm = initializeSearhForm();
		comp.employmentTypeForm.controls['name'].setValue(formValue.name);
		comp.employmentTypeForm.controls['comments'].setValue(formValue.comments);
		comp.employmentTypeForm.controls['description'].setValue(formValue.description);
		tick();
		expect(comp.loadSpin).toBeFalsy();
		comp.createFormSubmit(comp.employmentTypeForm.value);
		expect(comp.loadSpin).toBeFalsy();
	...
	it('Should createFormSubmit Subscribe body test when status false', fakeAsync(() => {
		...
		comp.employmentTypeForm = initializeSearhForm();
		comp.employmentTypeForm.controls['name'].setValue(formValue.comments);
		comp.employmentTypeForm.controls['comments'].setValue(formValue.description);
		comp.employmentTypeForm.controls['description'].setValue(formValue.name);
	...
	it('Should setpage function test', () => {
		...
		spyOn(comp, 'getBillingEmploymentType');
		spyOn(comp['selection'], 'clear');
		comp.employmentTypeForm = comp.initializeSearhForm();
		const pageInfo = {
			offset: 0,
		};
		comp['page'].pageNumber = pageInfo.offset;
		const pageNumber = comp['page'].pageNumber + 1;
		comp.employmentTypeForm.controls.name.setValue('Your name');
		comp.employmentTypeForm.controls.description.setValue('Your description');
		const filters = checkReactiveFormIsEmpty(comp.employmentTypeForm);
		comp['queryParams'] = {
		...
		expect(comp.getBillingEmploymentType).toHaveBeenCalled();
	});
	it('Should getBillingEmploymentType function test when subscribe throw error', fakeAsync(() => {
		const formValue: any = {
			comments: 'comment',
			description: 'desc',
			name: 'name',
		...
		comp.employmentTypeForm.controls['description'].setValue(formValue.name);
        spyOn(request_MockService, 'sendRequest').and.returnValue(throwError('error') );
		comp.getBillingEmploymentType(formValue);
        expect(request_MockService.sendRequest).toHaveBeenCalled();
	...
	it('Should Test isAllSelected', () => {
		comp.employmentTypeListing.length = 10;
		comp.totalrows = comp.employmentTypeListing.length;
	...
	it('Should Test patchEditValues', () => {
		...
		comp.employmentTypeListing = mockPlaceOfServiceList;
	...
	it('Should check Reset Button when employmentTypeSearchForm From Empty function Test', () => {
		comp.employmentTypeSearchForm = comp.initializeSearhForm();
		const retrunValue = isEmptyObject(comp.employmentTypeSearchForm.value);
		comp.checkInputs();
		expect(retrunValue).toBe(true);
	});
	it('Should check Reset Button disabled when employmentTypeSearchForm have values function Test', () => {
		comp.employmentTypeSearchForm = comp.initializeSearhForm();
		comp.employmentTypeSearchForm.controls.name.setValue('mock name');
		comp.employmentTypeSearchForm.controls.description.setValue('mock description');
		const retrunValue = isEmptyObject(comp.employmentTypeSearchForm.value);	
		comp.checkInputs();
	...
	it('Should Test closeModel when employmentTypeForm dirty and touched false', fakeAsync(() => {
		const ngbModalOptions: NgbModalOptions = {
			backdrop: 'static',
			keyboard: false,
			windowClass: 'modal_extraDOc unique',
		...
		spyOn(comp.employmentTypeForm,'reset');
		comp.employmentTypeForm.reset();
		comp.closeModel();
		comp['modalref'].close();
		expect(comp.employmentTypeForm.reset).toHaveBeenCalled();
	}));
	it('Should Test closeModel when employmentTypeForm dirty and touched true', fakeAsync(() => {
	...
	it('Should Test closeModel when employmentTypeForm dirty and touched true and subcribe throw error', fakeAsync(() => {
		// const spyOnMethod = spyOn(comp['CanDeactivateModelComponentService'], 'canDeactivate').and.returnValue(of([]));
	...
	it('Should getBillingEmploymentType with employmentTypeListing empty', fakeAsync(() => {
		const formValue: any = {
			comments: 'comments',
			description: 'description',
			name: 'name',
        ...
        tick(15000);
		discardPeriodicTasks();
		expect(comp.employmentTypeListing.length).toBe(0);
    }));
	it('Should Test updateForm function when subsribe run successfull',fakeAsync(()=>{
		...
		spyOn(request_MockService, 'sendRequest').and.returnValue(of(formValue).pipe(delay(1)));
		comp.updateForm(comp.employmentTypeForm);
        fixture.detectChanges();
		expect(comp.loadSpin).toBeTruthy();
        expect(request_MockService.sendRequest).toHaveBeenCalled();
        tick(15000);
	...
	it('Should Test updateForm function when subsribe run successfull and subscribe return error',fakeAsync(()=>{
		...
		comp.employmentTypeForm.controls.name.setValue('mock name');
		comp.employmentTypeForm.controls.description.setValue('mock description');
		spyOn(request_MockService, 'sendRequest').and.returnValue(throwError({error:{message:'Subcribe Error'}}));
		comp.updateForm(comp.employmentTypeForm);
        fixture.detectChanges();
        expect(request_MockService.sendRequest).toHaveBeenCalled();
        tick(15000);
	...
	it('Should Test updateForm function When employmentTypeForm not valid',()=>{
	...
	it('Should Test createFormSubmit function When employmentTypeForm not valid',()=>{
		spyOn(helper, 'touchAllFields');
		comp.employmentTypeForm = comp['fb'].group({
			id: [''],
			name: ['', Validators.required],
		...
		comp.createFormSubmit(comp.employmentTypeForm);
		expect(helper.touchAllFields).toHaveBeenCalled();
	});
	it('Should Test onFormSubmit function',()=>{
		...
		comp.employmentTypeForm.controls.name.setValue('mock name');
		comp.employmentTypeForm.controls.description.setValue('mock description');
		comp.onFormSubmit(comp.employmentTypeForm.value);
		comp.modelTitle = 'Add';
	...
	it('Should Test openModal function with row not undefined',()=>{
		...
		comp.employmentTypeForm.controls.name.setValue('mock name');
		comp.employmentTypeForm.controls.description.setValue('mock description');
		comp.openModal(undefined,comp.employmentTypeForm.value,3);
	...
	it('Should resetFilter',()=>{
		comp.employmentTypeSearchForm = comp['fb'].group({
			name: ['name'],
			description: ['desc'],
		});
		spyOn(comp.employmentTypeSearchForm,'reset');
		spyOn(comp.selection,'clear');
		spyOn(comp,'setpage');
		comp.resetFilter();
		expect(comp.employmentTypeSearchForm.reset).toHaveBeenCalled();
		expect(comp.selection.clear).toHaveBeenCalled();
		expect(comp.setpage).toHaveBeenCalled();
	});
	afterAll(() => {
