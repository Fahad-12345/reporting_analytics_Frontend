...
export class InvoiceListingComponent extends PermissionComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.totalRows = this.invoicesformatlist.length;
    this.invoicesformatlist = this.invoicesformatlist;
    ...
    this.invoiceListtingTable = this.localStorage.getObject('invoiceTableList' + this.storageData.getUserId());
  }
  ...
  ngAfterViewInit() {
		if (this.invoiceListTable?._internalColumns) {
			...
			this.columns.forEach(element => {
				if(this.invoiceListtingTable.length) {
					let obj = this.invoiceListtingTable.find(x => x?.header === element?.name);
			...
			if(this.invoiceListtingTable.length) {
				...
				this.invoiceListtingTable.forEach((item, index) => {
  ...
  setPage(pageInfo) {
    ...
    this.getInvoiceFormatList({...this.queryParams,...filters})
  }
  ...
  masterToggle(event) {
    this.isAllSelected()
      ? this.selection.clear()
      : this.invoicesformatlist.slice(0, this.totalRows).forEach((row) => this.selection.select(row));
  }
      ...
      this.requestService
        ...
        .subscribe(
          (data: HttpSuccessResponse) => {
            if (data.status) {
              this.loadSpin = false;
              this.invoicesformatlist = data.result && data.result.data ? data.result.data : [];
  ...
  saveUpdateInvoiceBuilder(templateId) {
    if(templateId){
      this.rou.navigate(['front-desk/masters/builder-invoice'],{ queryParams: {id: templateId,
	 isMaster:true
	}});
    }
    else{
  ...
  onDeleteInvioceFormat(id?){
    ...
    this.customDiallogService.confirm('Delete Confirmation?', `Are you sure you want to delete?`,'Yes','No')
        ...
        this.subscription.push(
          this.requestService.sendRequest(InvoiceBuilderEnumURLs.Invoice_builder_List_Delete,'DELETE',REQUEST_SERVERS.fd_api_url,{id:ids}).subscribe(
            (res)=>{
              this.selection.clear();
              this.setPage({ offset: this.page.pageNumber});
              this.invoicesformatlist = this.removeMultipleFromArr(this.invoicesformatlist,ids,'id');
              this.invoicesformatlist = this.invoicesformatlist
  ...
  previewTemplate(content,row){
    ...
    const ngbModalOptions: NgbModalOptions = {
      backdrop: 'static',
      keyboard: false,
      windowClass: 'modal_extraDOc invoice-prev',
      size:'lg'
    };
