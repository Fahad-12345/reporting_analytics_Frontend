...
import { BillingInsuranceBackendServicesService } from '@appDir/front-desk/masters/billing/insurance-master/services/billing-insurance-backend-services.service';
import { BillingInsuranceDataServiceService } from '@appDir/front-desk/masters/billing/insurance-master/services/billing-insurance-data-service.service';
import { RequestService } from '@appDir/shared/services/request.service';
import {
	BillingInsuranceModel,
	LocationsModel,
} from '@appDir/front-desk/masters/billing/insurance-master/models/BillingInsurance.Model';
...
import { LocationEditComponentComponent } from '@appDir/front-desk/masters/billing/insurance-master/Insurance/location-edit-component/location-edit-component.component';
...
export class AddClinicComponent implements OnInit {
	...
	constructor(
		...
		private referringService: ReferringPhysicianService,
	) {}
	...
	ngOnInit() {
		...
		this.isClinicLocationSave();
	}
	ngOnChanges() {
		this.addEditMode();
		// -----------------------------
		// -----------------------------
		this.getEditDataClinic();
	}
	addEditMode() {
		if (!this.id) {
	...
	openLocationForm(location?: LocationsModel, index?) {
		this.locationcount =
			this.insurance && this.insurance.insurance_locations
				? this.insurance.insurance_locations.length
				: 0;
		this.locationEditMode = true;
	...
	ngOnDestroy() {
		// this.referringService.locationModalClose$.unsubscribe();
		// this.referringService.locationModalSave$.unsubscribe();
		unSubAllPrevious(this.subscription);
	}
	// --------------------------------------
	// --------------------------------------
	AddClinicForm: FormGroup;
	@Output() closedAddNewClinicWindow = new EventEmitter();
	@Output() updateClinic = new EventEmitter();
	isShowAddLocationOnEditClinic = true;
	updatedClinicLocation;
	@Input() isEditMood: boolean = false;
	@Input() getEditClinicData: any = {};
	initClinicForm() {
	...
	closedAddNewClinic() {
		debugger;
		if (this.AddClinicForm.dirty && this.AddClinicForm.touched) {
			this.CanDeactivateModelComponentService.canDeactivate().then((res) => {
				if (res) {
					this.AddClinicForm.reset();
					this.closedAddNewClinicWindow.emit(false);
	...
	closeClinicLocationModal() {
		debugger;
		this.subscription.push(
		this.referringService.locationModalClose$.subscribe((result) => {
			debugger;
			if (result) {
				debugger;
				if(this.modalRef){
	...
	isClinicLocationSave() {
		debugger;
		this.subscription.push(
		this.referringService.locationModalSave$.subscribe((res) => {
			debugger;
			if (res) {
				this.updatedClinicLocation = res;
				this.saveClinit();
			}
		}));
	}
	getEditDataClinic() {
		debugger;
		if (!_.isEmpty(this.getEditClinicData) && this.isEditMood) {
			this.initClinicForm();
			this.isShowAddLocationOnEditClinic = false;
			setTimeout(() => {
				this.AddClinicForm.controls['clinic_name'].setValue(
					this.getEditClinicData && this.getEditClinicData.name ? this.getEditClinicData.name : '',
				);
				this.AddClinicForm.controls['id'].setValue(
					this.getEditClinicData && this.getEditClinicData.id ? this.getEditClinicData.id : '',
				);
				this.title = this.isEditMood ? 'Update' : 'Add';
			}, 200);
	...
	saveClinit() {
		debugger;
		let addNewClinicObject = {
			name: this.AddClinicForm.controls['clinic_name'].value,
			...this.updatedClinicLocation,
		};
		this.loading = true;
		this.referringService.AddNewClinicWithPrimaryLocation(addNewClinicObject).subscribe(
			(res) => {
				if (res && res.status) {
					if(this.modalRef) {
						this.modalRef.close();
					...
					this.closedAddNewClinicWindow.emit(false);
					this.referringService.nowGetAllClinics$.next(true);
					this.AddClinicForm.reset();
					this.referringService.isAdd_Update_Location$.next(false);
					this.referringService.locationModalSave$.next(false);
	...
	UpdateClinit() {
		debugger;
		let updateClinicObject = {
			clinic_id: this.AddClinicForm.controls['id'].value,
			clinic_name: this.AddClinicForm.controls['clinic_name'].value,
		};
		this.loading = true;
		this.referringService.UpdateClinic(updateClinicObject).subscribe(
			(res) => {
				if (res && res.status) {
					this.toaster.success(res.message, 'Success');
					this.updateClinic.emit(true);
					this.loading = false;
					console.log(res);
				} else {
					this.loading = false;
